...
def ReadHierarchicalVerilogToGraph(modulepath, topmodule_name):
        ...
        if(filename.endswith("rtlbb.v")):
            if(filename != (topmodule_name+'.rtlbb.v')):
                ...
                for i in input_port_list:
                    #                 IO_dict[gate_dict[i[0]]+existing_length] = i[0]
                    # check if the input is already exist in IO_dict
                    if(i[0] in IO_dict):
                        for idx, (e0, e1) in enumerate(edges):
                            if(e0 == gate_dict[i[0]]+existing_length):
                                # print("merged:", e0, " To:", IO_dict[i[0]])
                                edges[idx] = (IO_dict[i[0]], e1)
                            elif(e1 == gate_dict[i[0]]+existing_length):
                                # print("merged:", e1, " To:", IO_dict[i[0]])
                                edges[idx] = (e0, IO_dict[i[0]])
                        # nodes.remove(gate_dict[i[0]]+existing_length) ##remove merged node
                        gate_dict[i[0]] = IO_dict[i[0]] ##redirect connection
                        num_merge +=1
                        
                    else:
                        IO_dict[i[0]] = gate_dict[i[0]]+existing_length
                        IO_type_dict[i[0]] = 0
                
                for o in output_port_list:
                    # IO_dict[gate_dict[o[0]]+existing_length] = o[0]
                    IO_dict[o[0]] = gate_dict[o[0]]+existing_length
                    IO_type_dict[o[0]] = 1
        ...
        if(a_gate[0][0] in componentlist):
            for connection in a_gate[1:]:
                ...
                if(IO_type_dict[connection[0]]==1):
                    ...
                    output_node = IO_dict[connection[0]]
                ...
                ## if connect to components input.
                elif(IO_type_dict[connection[0]]==0):
                    if (not connection[1] in gate_dict):
                        ## add gate connecting to outputs of components to gate_dict 
                        gate_dict[connection[1]] = gate_length + 2 + existing_length
                        print("creating new gate:",connection[1],": ",gate_length + 2 + existing_length)
                    ...
                    input_node = IO_dict[connection[0]]
    ...
    for node in mips_node_list:
        gate_label[node] = len(componentlist)
    node_label = []
    graph_indicator=[]
    ...
    for idx,c in enumerate(componentlist):
        com_lib.append((idx,c))
    com_lib.append((len(componentlist),topmodule_name))
